    ...
    for prod in produtos:
        ...
        with st.container(border=True):
            ...
            if st.button("Editar", key=f"editar_{id_prod}"):
                ...
                st.session_state.produto_editando = id_prod
                st.rerun()

    if "produto_editar" in st.session_state:
        idp = st.session_state.produto_editar

        st.write("---")
        st.subheader("Editar Produto")

        atual = [p for p in produtos if p[0] == idp][0]
        _, nome_at, desc_at, qtd_at, valor_at = atual

        novo_nome = st.text_input("Nome", value=nome_at)
        nova_desc = st.text_area("Descrição", value=desc_at)
        nova_qtd = st.number_input("Quantidade", min_value=0, value=int(qtd_at))
        novo_valor = st.number_input("Valor (R$)", min_value=0.0, value=float(valor_at))
        novo_minimo = st.number_input("Quantidade mínima", min_value=0, value=1)

        categoria_escolhida = st.selectbox(
            "Categoria",
            options=list(mapa_categorias.keys()),
            format_func=lambda x: mapa_categorias[x]
        )

        if st.button("Salvar Alterações"):
            editar_produto(idp, novo_nome, nova_desc, nova_qtd, novo_valor, novo_minimo, categoria_escolhida)
...
def tela_editar_produto():
    ...
    prod = [p for p in produtos if p[0] == id_produto][0]
...
def tela_gestao_funcionarios():
    ...
    senha = st.text_input("Senha", type="password")
    acesso = st.selectbox("Nível de acesso", [0, 1], format_func=lambda x: "Gestor" if x == 1 else "Funcionário")

    if st.button("Cadastrar", key="cad_func"):
        if nome and usuario and senha:
            cadastrar_funcionario(nome, usuario, senha, acesso)
